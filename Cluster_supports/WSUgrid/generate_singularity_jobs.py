...
def generate_event_folders(workingFolder, clusterName, eventId,
                           ...
                           bayesParamFile, eventId0, nEvents,
...
""".format(singularityRepoPath, executeScriptName, workFolderPath,
           parameterFileName, eventId0, nEvents, nThreads, seed,
           bayesParamFile.split('/')[-1]))
    if clusterName == "anvil":
        script.write("""

    source $PROJECT/IPGlasmaFramework/Cluster_supports/Anvil/bashrc
""")
    script.write("""
    mkdir -p temp
...
def main():
    ...
    try:
        ...
        n_jobs = args.n_jobs
        n_event_per_job = args.n_event_per_job
